...
5. Save the file as **UTF-8 with a BOM** (byte-order mark). Emoji icons and any non-ASCII rely on
   this. A script running under `[ScriptHost('auto')]` or `[ScriptHost('powershell')]` may execute
   under **Windows PowerShell 5.1**, which reads a BOM-less file as the machine's legacy ANSI
   codepage — an emoji like `[ScriptIcon('🗂️')]` then decodes to garbage (`ðŸ—‚`) and the script
   fails to parse ("The string is missing the terminator"). The BOM makes 5.1 read it as UTF-8;
   pwsh 7 and PaletteShell's own parser already handle either. Plain UTF-8 without a BOM only works
   if the script is guaranteed to run under pwsh 7 — don't rely on that.
...
| Mode | Behavior |
|------|----------|
| `None` | Run silently; a "Script completed" toast is shown. With no `[ScriptTimeout]` this is **not waited** (output is not captured). |
| `Toast` | Wait, capture stdout, show it in the toast. |
| `Clipboard` | Wait, capture stdout, copy it to the clipboard. |
| `Open` | Wait, open the first non-empty stdout line as a URL, file, or folder path. |
| `File` | Wait, write stdout to a temp file and open it in the user's editor. Add an extension hint after a colon: `File:csv`, `File:json`, etc. Best for large/structured output. |

**In-palette (display)** — open a page that **stays open** and renders the output.
...
- [ ] Saved as **UTF-8 with a BOM** (so an emoji icon survives a Windows PowerShell 5.1 run); icon is a single emoji/glyph.
